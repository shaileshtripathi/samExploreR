...
For samExploreR package several functions of Rsubread were altered.
Modified Rsubread functions are marked with following statements:

// Modified code 
// end modified code

Authors: Alexey Stupnikov and Shailesh Tripathi
...
// Modified code, declaration of random function 
double my_boot=1;
int getRand(double probability) {
...
void fc_write_final_gene_results(fc_thread_global_context_t * global_context, int * et_geneid, char ** et_chr, long * et_start, long * et_stop, unsigned char * et_strand, const char * out_file, int features, unsigned int ** column_numbers, char * file_list, int n_input_files, fc_feature_info_t * loaded_features, int header_out, double d_boot)
{
	int flag_boot, tmp_flag=0;	
	int xk1;
...
int readSummary(int argc,char *argv[]){
	...
	if(isGeneLevel)
		fc_write_final_gene_results(&global_context, geneid, chr, start, stop, sorted_strand, argv[3], nexons,  table_columns, argv[2], n_input_files , loaded_features, isCVersion, d_boot);
	else
