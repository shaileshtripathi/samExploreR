...
\usepackage{dsfont}
%\VignetteIndexEntry{samExploreR}
...
1)	ggplot2: package available at
...
2)	pBrackets: package available at
http://cran.r-project.org/web/packages/pBrackets/index.html
...
3)	colorspace: package available at
...
The function $samExplore$, which is our modification of the featureCounts function of \cite{liao2013featurecounts}, performs a summerization of reads to genomic features of annotation. 
...
> inpf <- paste(inpf,"/","Test.sam",sep="")
 ...
 > x <- samExplore(files=inpf,annot.inbuilt="mm9",GTF.featureType="gene",
 GTF.attrType="gene_id", subsample_d = 0.8, N_boot=5)
...
The function $exploreRob$ takes as input argument a data frame with the format shown in Tab. \ref{tab:expRob}.
\begin{table}[h!]
...
\end{tabular}
\caption{Input argument for the function $exploreRob$. \label{tab:expRob}}
\end{table}
In this table, the first column provides the labels for the annotation (lbl) used for the analysis, the second column gives the $f$ value and the third column provides the value of the metric to be explored, e.g., the number of differentially expressed genes. 

The function $exploreRob$ splits this data frame up by considering only entries for one specific type of annotation. Then an ANOVA test is run for the groups of replicates that correspond to a given list of $f$ values, see Fig. \ref{fig1}.
...
$exploreRep$ takes as input a data frame of form shown in Tab. \ref{tab:expRep}.
\begin{table}[h!]
\begin{tabular}{ll*{6}{c}r}
&Label & Variable& Value\\
.&.&.&.\\
...
\end{table}
Here the first column provides the labels for the annotation used for the analysis, the second column gives the $f$ value and the third column provides the value of the metric to be explored, e.g., the number of differentially expressed genes. 

$exploreRep$ splits this data frame up to consider only results for one $f$ value. ANOVA test is run for groups of replicates with corresponding to given list of annotation labels , Fig \ref{fig1}.
...
The input argument of this function should be a {\it{data.frame}} object containing three columns with the names - {\it{Label}}, {\it{Variable}}, {\it{Value}}.
...
@
\caption{\label{fig1} Boxplot of the number of differentially expressed genes for different sequence-depths $f$.  }
}
\end{center}
\end{figure}
...
@
\caption{\label{fig2} Boxplot of the number of differentially expressed genes for different sequence-depths $f$.  }
}
\end{center}
\end{figure}
